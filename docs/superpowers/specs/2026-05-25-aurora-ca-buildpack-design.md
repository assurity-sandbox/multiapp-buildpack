...
- `bin/supply`: validates arguments, reads `VCAP_SERVICES`, extracts unique
  ...
  scripts.
- `lib/vcap_ca_urls.rb`: parses `VCAP_SERVICES` JSON using Ruby's standard JSON
  library and emits unique Aurora PostgreSQL CA URLs.
...
- Missing required tools: fail staging when `ruby`, `curl`, or `openssl` is
  unavailable.
